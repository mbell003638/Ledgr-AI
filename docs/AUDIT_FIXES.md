...
## Round 15 — AI party-role safety and Capital Account recovery (2026-08-13)

| User report | Root cause | Fix | Files |
| --- | --- | --- | --- |
| Voice command "paid 100 to Amit" posted a Supplier payment even though Amit was an existing Capital Account | Voice save paths used fuzzy/substring party matching and auto-created missing Suppliers or Customers, bypassing the manual account form's duplicate-name validation | Added shared role-aware resolution against existing Suppliers, Customers, and Capital Accounts before confirmation. Ambiguous commands now ask a counter-question; bill misclassification cannot bypass resolution; voice never auto-creates a payment party. Exact matching is required. | `voicePartyResolution.ts` (new), `voice.tsx`, `VoiceFab.tsx` |
| Deleting the wrong Supplier payment still left Amit's Capital Statement unavailable | Business Accounts merged same-name party/member rows and retained the Supplier ID while displaying the row as a Capital Account; the detail screen also rejected valid migrated member rows based only on stale book style | Supplier/Customer and Capital Account rows now remain separate and preserve their authoritative IDs. Capital Statement access is determined by the real member record, so a stale same-name Supplier no longer blocks the Capital Account. Reversing the wrong payment keeps the ledger balanced; no book reset is required. | `(tabs)/suppliers.tsx`, `investor/[id].tsx`, `investorLedgerService.ts` |
| AI/API paths could recreate the same collision even though the manual New Supplier screen rejected it | Duplicate protection existed only at selected UI/API layers, not at the accounting repository write boundary | Capital Account name protection is enforced in the API, application service, transaction party resolver, and repository. Customer/Supplier creation or rename to a Capital Account's normalized name is rejected, while harmless edits to an already-stale party remain possible. | `api.ts`, `appService.ts`, `repository.ts` |
| Voice confirmation could hide or lose the payment account | Supplier payments and withdrawals were saved as Cash even when AI parsed Bank, Card, or UPI/mobile | Confirmation now displays the payment method and postings preserve Cash/Bank/Card/Mobile; UPI normalizes to Mobile. Named cash-sale receipts retain the exact existing Customer ID instead of creating a duplicate. | `voice.tsx`, `VoiceFab.tsx`, `appService.ts`, `investorLedgerService.ts` |

Regression coverage reproduces the affected state: a Capital Account plus a stale same-name Supplier, a wrong Supplier payment, reversal/deletion, Capital Statement access, and balanced reconciliation. It also covers ambiguous-role questions, bill-parser bypass attempts, exact-name matching, and repository-level name protection.

Validation after round 15: **63/63 Jest suites, 490 tests passing; lint clean; `tsc --noEmit` clean; Expo Doctor 18/18; no high or critical production dependency vulnerabilities.**
