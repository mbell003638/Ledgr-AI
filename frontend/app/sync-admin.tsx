...
export default function SyncAdminScreen() {
  ...
  const [generatedCode, setGeneratedCode] = useState<{ code: string; expiresAt: string; role: string; locationIds: string[] } | null>(null);
  ...
  const createCode = async () => { setBusy(true); setMessage(''); try { const result = await api.createSyncEnrollmentCode(codeRole as Exclude<Membership['role'], 'owner'>, codeLocations, 15); setGeneratedCode(result); setMessage('One-time code created. Share it only with the intended user; it expires in 15 minutes and is shown only here.'); } catch (error: any) { setMessage(error?.message || 'Could not create a one-time enrollment code.'); } finally { setBusy(false); } };
