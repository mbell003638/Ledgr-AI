...
export default function SyncSettingsScreen() {
  ...
  return <SafeAreaView style={styles.container} edges={['top']}>
    <View style={styles.header}><Pressable onPress={() => router.back()} accessibilityRole="button" accessibilityLabel="Go back"><Ionicons name="arrow-back" size={24} color={theme.color.onSurface} /></Pressable><ScreenHeader style={styles.headerScreen} title="Self-hosted Sync" subtitle="Optional offline-first collaboration" titleStyle={styles.headerTitle} /></View>
    ...
    <ScrollView ref={scrollRef} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" showsVerticalScrollIndicator={false}>
      <View style={styles.card}><Text style={styles.title}>Your device stays local first</Text><Text style={styles.hint}>Writes commit locally immediately. Enrollment obtains the Business Account epoch from your server; recovery never merges raw SQLite files.</Text>
        ...
        <Text style={styles.label}>Manual access token (development fallback)</Text><TextInput value={token} onChangeText={setToken} autoCapitalize="none" secureTextEntry returnKeyType="done" onSubmitEditing={Keyboard.dismiss} onFocus={() => setTimeout(() => scrollRef.current?.scrollToEnd({ animated: true }), 120)} placeholder="Stored only in SecureStore" placeholderTextColor={theme.color.muted} style={styles.input} />
