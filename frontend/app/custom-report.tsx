  ...
  const buildStyledHtml = (text: string) => {
    ...
    for (const line of lines) {
      ...
      if (trimmed === trimmed.toUpperCase() && trimmed.length > 3 && !trimmed.includes(':')) {
        ...
        tableRows += `<tr><td colspan="2">${trimmed}</td></tr>`;
      }
    }
    if (tableRows) {
      bodyHtml += `<table><thead><tr><th>Description</th><th class="num">Amount</th></tr></thead><tbody>${tableRows}</tbody></table>`;
...
<head>
  ...
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 36px; color: #1C221F; background: #fff; }
    .header { border-bottom: 3px solid #1C4030; padding-bottom: 16px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: flex-end; }
    .title { font-size: 24px; font-weight: 800; color: #1C4030; margin: 0; letter-spacing: -0.5px; }
    .subtitle { font-size: 13px; color: #667069; margin-top: 4px; font-weight: 500; }
    .period-badge { background: #E8F0EC; color: #1C4030; font-size: 12px; font-weight: 700; padding: 6px 14px; border-radius: 6px; }
    .section-title { font-size: 14px; font-weight: 700; color: #1C4030; background: #F2F6F4; padding: 8px 12px; border-left: 4px solid #1C4030; margin-top: 24px; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px; border-radius: 0 4px 4px 0; }
    table { width: 100%; border-collapse: collapse; margin-bottom: 16px; font-size: 13px; }
    th { background: #FAFCFA; color: #4A5550; text-align: left; padding: 8px 12px; font-weight: 600; border-bottom: 1px solid #DCE2DE; }
    td { padding: 9px 12px; border-bottom: 1px solid #EAEFEA; color: #2C3531; }
    .num { text-align: right; font-weight: 600; }
    .footer { margin-top: 40px; padding-top: 16px; border-top: 1px solid #E2E8E4; font-size: 11px; color: #8A938E; display: flex; justify-content: space-between; }
  </style>
</head>
<body>
  <div class="header">
    <div>
      <h1 class="title">LEDGR FINANCIAL STATEMENT</h1>
  ...
  ${bodyHtml}
  <div class="footer">
    <span>Generated by Ledgr AI Accounting</span>
