...
const EXPECTED_AAD = `${ENCRYPTED_BACKUP_FORMAT}:v${ENCRYPTED_BACKUP_VERSION}`;
...
function bytesToBase64(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < bytes.length; i += 3) {
    const a = bytes[i]; const b = i + 1 < bytes.length ? bytes[i + 1] : 0; const c = i + 2 < bytes.length ? bytes[i + 2] : 0;
    const n = (a << 16) | (b << 8) | c;
...
function assertEnvelope(encrypted: EncryptedBackupEnvelope): void {
  ...
  if (!encrypted.kdfParams || encrypted.kdfParams.N !== KDF_N || encrypted.kdfParams.r !== KDF_R || encrypted.kdfParams.p !== KDF_P || encrypted.kdfParams.dkLen !== KEY_LENGTH) throw new Error('Encrypted backup has unsupported key-derivation parameters.');
  if (encrypted.aad !== EXPECTED_AAD) throw new Error('Encrypted backup authentication metadata is invalid.');
...
export async function encryptBackup(data: unknown, passphrase: string): Promise<EncryptedBackupEnvelope> {
  ...
  try {
    const ciphertext = gcm(key, nonce, utf8(EXPECTED_AAD)).encrypt(utf8(JSON.stringify(data)));
    return {
      format: ENCRYPTED_BACKUP_FORMAT, version: ENCRYPTED_BACKUP_VERSION, cipher: 'AES-256-GCM', kdf: 'scrypt',
      kdfParams: { N: KDF_N, r: KDF_R, p: KDF_P, dkLen: KEY_LENGTH },
      salt: bytesToBase64(salt), nonce: bytesToBase64(nonce), aad: EXPECTED_AAD,
      ciphertext: bytesToBase64(ciphertext), integrity: await backupIntegrityHash(data), createdAt: new Date().toISOString(),
    };
  } finally { key.fill(0); }
}
...
export async function decryptBackup(encrypted: EncryptedBackupEnvelope, passphrase: string): Promise<any> {
  ...
  try {
    plaintext = gcm(key, nonce, utf8(EXPECTED_AAD)).decrypt(base64ToBytes(encrypted.ciphertext));
  } catch {
