...
export const api = {
  ...
  postV2OpeningBalances: async (input: { date?: string; cash: number; inventory: number; otherAssets?: number; accountsPayable?: number; otherLiabilities?: number; ownerCapital?: number; retainedEarnings?: number; memo?: string }) => {
    const runner = activeSqlRunner();
    if (!runner) throw new Error('V2 accounting requires SQLite storage');
  ...
  updateV2OpeningBalances: async (input: { date?: string; cash: number; inventory: number; otherAssets?: number; accountsPayable?: number; otherLiabilities?: number; ownerCapital?: number; retainedEarnings?: number; memo?: string }) => {
    const runner = activeSqlRunner();
    if (!runner) throw new Error('V2 accounting requires SQLite storage');
