...
export function buildAnalyzeDocumentPrompt(pastedText?: string): string {
  ...
  let prompt =
    ...
    "'receipt_in' (money received), 'payment_out' (money paid out to a supplier/party), 'expense' (operating cost), or 'capital_contribution' only when the document explicitly proves an owner/partner capital deposit. " +
