...
export async function getV2FeatureDisableBlockers(db: SqlRunner, bookId: string): Promise<FeatureDisableBlockers> {
  ...
  const sourceRows = await db.all<{ type: string; n: number }>(`SELECT type,COUNT(*) AS n FROM v2_sources WHERE book_id=? AND ${LIVE_SOURCE} GROUP BY type`, [bookId]);
  ...
  const block = (key: FeatureKey, detail: string) => { blockers[key] = `Cannot disable ${FEATURE_LABELS[key]} because this book still has ${detail}. Delete or reverse those entries first, then try again.`; };
  ...
  const [inventoryCounts, closeSnapshots, inventoryLedgerActivity, productActivity, payrollData, locationActivity] = await Promise.all([
    ...
    db.first<{ n: number }>(`SELECT COUNT(*) AS n FROM v2_journal_lines l JOIN v2_accounts a ON a.id=l.account_id JOIN v2_journal_entries j ON j.id=l.journal_id LEFT JOIN v2_sources s ON s.id=j.source_id WHERE j.book_id=? AND a.code='1200' AND (s.id IS NULL OR (${LIVE_SOURCE.replace(/metadata/g, 's.metadata')}))`, [bookId]),
    db.first<{ n: number }>(`SELECT COUNT(*) AS n FROM v2_stock_moves m LEFT JOIN v2_sources s ON s.id=m.source_id WHERE m.book_id=? AND (s.id IS NULL OR (${LIVE_SOURCE.replace(/metadata/g, 's.metadata')}))`, [bookId]),
    db.first<{ n: number }>('SELECT (SELECT COUNT(*) FROM v2_employees WHERE book_id=? AND archived=0) + (SELECT COUNT(*) FROM v2_pay_runs WHERE book_id=?) AS n', [bookId, bookId]),
    db.first<{ n: number }>(`SELECT (SELECT COUNT(*) FROM v2_sources WHERE book_id=? AND (location_id IS NOT NULL OR type IN ('location_cash_transfer','location_stock_transfer')) AND ${LIVE_SOURCE}) + (SELECT COUNT(*) FROM v2_inventory_counts WHERE book_id=? AND location_id IS NOT NULL) + (SELECT COUNT(*) FROM v2_journal_lines l JOIN v2_journal_entries j ON j.id=l.journal_id LEFT JOIN v2_sources s ON s.id=j.source_id WHERE j.book_id=? AND l.location_id IS NOT NULL AND (s.id IS NULL OR (${LIVE_SOURCE.replace(/metadata/g, 's.metadata')}))) + (SELECT COUNT(*) FROM v2_stock_moves m LEFT JOIN v2_sources s ON s.id=m.source_id WHERE m.book_id=? AND m.location_id IS NOT NULL AND (s.id IS NULL OR (${LIVE_SOURCE.replace(/metadata/g, 's.metadata')}))) AS n`, [bookId, bookId, bookId, bookId]),
  ]);
  if (Number(inventoryCounts?.n || 0) + Number(closeSnapshots?.n || 0) + Number(inventoryLedgerActivity?.n || 0) > 0) block('inventory', 'inventory ledger activity, physical stock counts, or closed inventory periods');
...
export async function assertFeatureDisableAllowed(db: SqlRunner, bookId: string, previous: readonly FeatureKey[], next: readonly FeatureKey[]): Promise<void> {
